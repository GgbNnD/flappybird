...
最终基础最佳模型保存在 \texttt{q\_best.pkl} 中，50 局无步数限制评估的平均得分为 $\mathbf{165.18}$，最高分为 $\mathbf{984}$。在 bonus 实验中：bonus1 改良状态（\texttt{enhanced\_top}）将均分提升至 $\mathbf{275.40}$；bonus2 改良奖励（\texttt{shaped\_v2}）将均分提升至 $\mathbf{311.16}$；两者结合的组合模型均分达到 $\mathbf{571.50}$，最高分 $\mathbf{2620}$，保存在 \texttt{q\_combined\_best.pkl} 中。所有数值均以无步数上限评估为准。
...
\begin{table}[H]
	...
	\begin{tabular}{cclp{4cm}}
		...
		$\begin{bmatrix}
			...
			player\_v
		\end{bmatrix}$ &
		基础最佳方案。相比示例方法新增"小鸟与管道缺口中心的垂直差"。这直接表达了"飞向缺口中心"的目标。\\
		\midrule
		\texttt{enhanced\_top} & 5 维 &
		$\begin{bmatrix}
			pipe\_x - player\_x \\
			player\_y - gap\_center \\
			pipe\_bottom - player\_y \\
			...
			player\_y - pipe\_top
		\end{bmatrix}$ &
		\textbf{bonus1 最终方案}。在 enhanced 基础上新增"到上管道距离"。仅增加 1 维，Q 表规模可控（$\sim$21k），但提供了间隙大小和上边界信息，使小鸟能在狭窄间隙中更精准地控制高度。100k 局训练后均分 $275.40$，超越 enhanced 的 $165.18$。\\
		\midrule
		\texttt{bonus} & 6 维 &
		$\begin{bmatrix}
			pipe\_x - player\_x \\
			...
			player\_rot
		\end{bmatrix}$ &
		在增强状态基础上加入"到上管道距离"和"旋转角"。但 6 维导致 Q 表极度稀疏（$\sim$26k），且旋转角噪声大、信息增益低，复训后仅 $67.74$。\\
		\bottomrule
	\end{tabular}
\end{table}
...
\subsection{原版 bonus 复训与问题分析}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.85\textwidth]{bonus_retrain.png}
	\caption{Bonus 复训平均分（原版方案）}
	\label{fig:bonus_retrain}
\end{figure}

原版 bonus 方案的三组复训均分（$67.74$–$122.30$）均低于基础参数实验的最佳得分（$165.18$）。其中 \texttt{bonus + death\_penalty}（6 维含旋转角）降至 $67.74$，数据稀疏严重；\texttt{enhanced + shaped}（原版奖励塑形）降至 $109.04$，速度惩罚和扇动惩罚产生了反效果。
...
结论：\texttt{enhanced\_top} 状态和 \texttt{shaped\_v2} 奖励各自独立优于基础方案，两者结合效果更佳。最终提交模型：\texttt{q\_best.pkl}（基础）、\texttt{q\_bonus1\_best.pkl}（bonus1）、\texttt{q\_bonus\_best.pkl}（bonus2）、\texttt{q\_combined\_best.pkl}（组合）。
...
\begin{enumerate}
	...
	\item \textbf{最终提交}：基础最佳模型 \texttt{q\_best.pkl}（enhanced + death\_penalty）、bonus1 模型 \texttt{q\_bonus1\_best.pkl}、bonus2 模型 \texttt{q\_bonus\_best.pkl}、组合最佳模型 \texttt{q\_combined\_best.pkl}。注意：旧版报告因评估阶段设置了 5000 步步数上限，导致分数被截顶。本报告已重新进行无步数限制评估，所有数据均为模型的真实表现。
\end{enumerate}
