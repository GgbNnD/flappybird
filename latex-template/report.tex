...
最终基础最佳模型保存在 \texttt{q\_best.pkl} 中，50 局无步数限制评估的平均得分为 $\mathbf{165.18}$，最高分为 $\mathbf{984}$。为反映模型真实能力，本章及后续章节所有数值均以无步数上限复评结果为准（旧报告因评估阶段设置了 5000 步上限而导致分数被截顶）。
...
\begin{enumerate}
	...
	\item 扩展状态 \texttt{bonus}（平均分 $115.90$）在初筛中表现同样出色，与 \texttt{enhanced} 仅差 $4.3$ 分。但复训后平均分降至 $67.74$（详见第 \ref{sec:bonus_retrain} 节），这是因为状态维度从 4 维增加到 6 维后，离散状态空间面积扩大，同等训练局数下每个状态—动作对的访问次数减少（即数据稀疏问题），导致策略稳定性变差。
\end{enumerate}
...
\begin{table}[H]
	...
	\begin{tabular}{lcccc}
		...
		\midrule
		enhanced + death\_penalty & $120.20$ & $122.30$ & $634$ & $2$ \\
		enhanced + shaped       & $119.95$ & $109.04$ & $482$ & $1$ \\
		bonus + death\_penalty   & $115.90$ & $67.74$ & $331$ & $1$ \\
		\bottomrule
	\end{tabular}
\end{table}

然而在 100000 局复训后（图 \ref{fig:bonus_retrain}），奖励塑形方案的平均分（$109.04$）略低于基础方案（$122.30$），也低于全局最佳模型 $q\_best.pkl$ 的 $165.18$。可能原因有两方面：
\begin{enumerate}
...
图 \ref{fig:bonus_retrain} 显示，三组复训得分均低于基础参数实验的复训得分。其中 \texttt{bonus + death\_penalty}（扩展状态）下降最多，从初筛 $115.90$ 跌至复训 $67.74$，印证了高维状态带来的数据稀疏问题。\texttt{enhanced + shaped}（奖励塑形）复训得分 $109.04$，略低于 \texttt{enhanced + death\_penalty}（$122.30$），说明奖励塑形在长周期训练中没有带来稳定的增益。
...
\begin{enumerate}
	\item \textbf{Q-Learning 的有效性}：Q-Learning 能够在离散状态空间中逐步学习状态—动作价值，使智能体在无需了解游戏物理机制的前提下，通过试错获得穿越管道的有效策略。最终最佳模型 50 局无步数限制评估平均 $165.18$ 分，最高 $984$ 分，远超初筛时期多数参数组合的分值。
	...
	\item \textbf{最终模型}：以 $\alpha=0.7, \gamma=0.95, \varepsilon=0.0, obs\_mul\_factor=30$、状态模式 \texttt{enhanced}、奖励模式 \texttt{death\_penalty} 训练的 \texttt{q\_best.pkl} 为最终提交模型。注意：旧版报告因评估阶段设置了 5000 步步数上限，导致分数被截顶（报告的最高分 132 实为该上限的产物）。本报告已重新进行无步数限制评估，数据为模型的真实表现。
\end{enumerate}
