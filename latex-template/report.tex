...
最终基础最佳模型保存在 \texttt{q\_best.pkl} 中，50 局固定评估的平均得分为 $\mathbf{90.52}$，最高分为 $\mathbf{132}$。
...
\begin{table}[H]
	...
	\begin{tabular}{ll}
		...
		\texttt{q\_best.pkl} & 最终基础最佳 Q 表模型 \\
		\texttt{q\_bonus\_best.pkl} & bonus 对比实验中的最佳模型 \\
...
\subsection{状态表示设计（bonus1）}

\subsubsection{环境观测值 $obs$}
...
\begin{table}[H]
	...
	\begin{tabular}{cclp{4cm}}
		\toprule
		\texttt{state\_mode} & 维度 & 特征向量 & 设计思路 \\
		\midrule
		\texttt{example} & 3 维 &
		$\begin{bmatrix}
			pipe\_x - player\_x \\
			...
			player\_v
		\end{bmatrix}$ &
		作业示例方法。关注水平距离、到下管道的垂直距离和小鸟速度，是最基本的状态表示。\\
		\midrule
		\texttt{enhanced} & 4 维 &
		$\begin{bmatrix}
			pipe\_x - player\_x \\
			...
			player\_v
		\end{bmatrix}$ &
		\textbf{最终最佳方案}。相比示例方法新增"小鸟与管道缺口中心的垂直差"。这直接表达了"飞向缺口中心"的目标，因为穿越管道需要小鸟对准缺口而非单纯关注下管道。\\
		\midrule
		\texttt{bonus} & 6 维 &
		$\begin{bmatrix}
			pipe\_x - player\_x \\
			...
			player\_rot
		\end{bmatrix}$ &
		在增强状态基础上进一步加入"到上管道距离"和"小鸟旋转角"。上下管道边界对称信息有助于判断碰撞危险，旋转角反映飞行姿态，提供更完整的飞行状态描述。\\
		\bottomrule
	\end{tabular}
\end{table}


\subsubsection{与示例方法的性能比较}
...
\begin{enumerate}
	\item 示例状态 \texttt{example}（平均分 $63.70$）的性能明显低于增强状态 \texttt{enhanced}（平均分 $120.20$）。核心原因在于 \texttt{example} 仅保留了"与下管道距离"，忽略了小鸟在整个管道间隙中的相对位置。实际上，如果小鸟位于间隙顶部，即便下管道距离合适，仍然可能撞到上管道。\texttt{enhanced} 通过加入"到缺口中心的垂直差"，使状态能够区分小鸟在间隙上部和下部的不同情况，因此得分接近翻倍。

	\item 扩展状态 \texttt{bonus}（平均分 $115.90$）在初筛中表现同样出色，与 \texttt{enhanced} 仅差 $4.3$ 分。但复训后平均分降至 $53.84$（详见第 \ref{sec:bonus_retrain} 节），这是因为状态维度从 4 维增加到 6 维后，离散状态空间面积扩大，同等训练局数下每个状态—动作对的访问次数减少（即数据稀疏问题），导致策略稳定性变差。
\end{enumerate}

结论：\texttt{enhanced} 状态表示在表达能力和 Q 表规模之间取得了最佳平衡，被选作最终模型的默认状态模式。

\subsection{奖励分配设计（bonus2）}

\subsubsection{环境原始奖励}
...
\subsubsection{与原始奖励方案的比较}

在初筛实验中（30000 局训练，表 \ref{tab:bonus_screen}），\texttt{enhanced + shaped} 平均分为 $119.95$，非常接近 \texttt{enhanced + death\_penalty} 的 $120.20$，说明奖励塑形没有偏离原始目标，且能提供更细密的飞行姿态反馈。

\begin{table}[H]
	\centering
	\caption{Bonus 初筛与复训结果}
	\label{tab:bonus_screen}
	\begin{tabular}{lcccc}
		\toprule
		配置 & 初筛均分 & 复训均分 & 最高分 & 最低分 \\
		\midrule
		enhanced + death\_penalty & $120.20$ & $78.86$ & $132$ & $2$ \\
		enhanced + shaped       & $119.95$ & $73.84$ & $132$ & $1$ \\
		bonus + death\_penalty   & $115.90$ & $53.84$ & $132$ & $1$ \\
		\bottomrule
	\end{tabular}
\end{table}

然而在 100000 局复训后（图 \ref{fig:bonus_retrain}），奖励塑形方案的平均分（$73.84$）略低于基础方案（$78.86$），也低于全局最佳模型 $q\_best.pkl$ 的 $90.52$。可能原因有两方面：
\begin{enumerate}
	\item 塑形系数的权重可能偏保守，使得中心奖励（最大仅 $+0.2$）在环境存活奖励（每帧 $+0.1$）面前不够突出；
	\item 奖励塑形引入了更多的局部反馈信号，在某些情况下可能引导智能体收敛到次优策略——例如过于贴近间隙中心而失去通过某些偏置管道的最佳路径。
\end{enumerate}

结论：bonus2 的奖励塑形在早期训练中能加速收敛，提供了除死亡/存活外的更丰富反馈；但最终复训评估中原死亡惩罚方案更为稳定，因此最终提交模型保留基础奖励方案。
...
每组初筛训练 $30000$ 局、评估 $20$ 局；前 $3$ 组复训 $100000$ 局、评估 $50$ 局。训练每局最多 $1000$ 步，评估每局最多 $5000$ 步。
...
\begin{table}[H]
	...
	\begin{tabular}{ccccccc}
		...
		\midrule
		基线参数 & $0.7$ & $0.95$ & $0.00$ & $\mathbf{90.52}$ & $3$ & $132$ \\
		低速·快折扣·小探索 & $0.5$ & $0.90$ & $0.05$ & $82.48$ & $2$ & $132$ \\
		中速·慢折扣·小探索 & $0.6$ & $0.99$ & $0.05$ & $51.02$ & $4$ & $132$ \\
		\bottomrule
	\end{tabular}
\end{table}
...
如图 \ref{fig:best_scores} 所示，50 局评估分数存在较大波动（标准差 $46.38$），最低 $3$ 分，最高 $132$ 分。这是因为 Flappy Bird 中管道序列随机生成，且一次错误 flap 可能迅速导致死亡。然而，模型在 50 局中共有 $21$ 局达到最高分 $132$（占比 $42\%$），且平均分超过 $90$，说明 Q 表已经学到了较有效的过管策略。第 3 局得分为 $3$ 的低分局可能是由于管道序列特别苛刻，而早期离散化带来的信息损失使其无法精细处理边缘情况。
...
\begin{itemize}
	\item $\alpha = 0.7$ 的基线参数表现最好（复训均分 $90.52$），$\alpha = 0.5$ 紧随其后（$82.48$）。这说明在该任务中，较大学习率有利于快速吸收新样本，尤其是在 100000 局的训练量下。
...
\begin{itemize}
	...
	\item $\varepsilon = 0.05$ 的配置仍能找到可用策略（如 $a=0.5, g=0.90, \varepsilon=0.05$ 复训均分 $82.48$），但总体弱于基线。
	\item $\varepsilon = 0.10$ 的配置几乎全部表现最差，如 $a=0.7, g=0.99, \varepsilon=0.10$ 仅 $3.80$ 分（倒数第二），说明过强的随机探索严重干扰了 Q 表的收敛。
\end{itemize}

\subsubsection{状态离散系数 \texttt{obs\_mul\_factor}}
...
\section{Bonus 实验完整结果}

\subsection{初筛结果}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.85\textwidth]{bonus_screen.png}
	\caption{Bonus 状态与奖励初筛平均分}
	\label{fig:bonus_screen}
\end{figure}

图 \ref{fig:bonus_screen} 显示，Bonus 初筛中前三名均为 $\varepsilon = 0$ 的配置（增强/扩展状态 + 死亡惩罚/奖励塑形），初筛均分在 $111$–$120$ 之间，远高于其后使用探索的配置。这与基础参数实验的 $\varepsilon = 0$ 优势结论一致。

\subsection{复训结果}
\label{sec:bonus_retrain}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.85\textwidth]{bonus_retrain.png}
	\caption{Bonus 复训平均分}
	\label{fig:bonus_retrain}
\end{figure}

图 \ref{fig:bonus_retrain} 显示，三组复训得分均低于基础参数实验的复训得分。其中 \texttt{bonus + death\_penalty}（扩展状态）下降最多，从初筛 $115.90$ 跌至复训 $53.84$，印证了高维状态带来的数据稀疏问题。\texttt{enhanced + shaped}（奖励塑形）下降至 $73.84$，略低于 \texttt{enhanced + death\_penalty}（$78.86$），说明奖励塑形在长周期训练中没有带来稳定的增益。
...
\begin{enumerate}
	\item \textbf{Q-Learning 的有效性}：Q-Learning 能够在离散状态空间中逐步学习状态—动作价值，使智能体在无需了解游戏物理机制的前提下，通过试错获得穿越管道的有效策略。最终最佳模型 50 局平均 $90.52$ 分，远高于基线参数的 $32.2$ 分。

	\item \textbf{状态表示的重要性}：加入"小鸟与管道缺口中心的垂直差"是提升性能的关键因素。\texttt{enhanced} 状态比作业示例状态 \texttt{example} 得分提升约 $2$ 倍（$120.20$ vs $63.70$）。过于丰富的状态表示（\texttt{bonus}）虽然初筛表现佳，但复训稳定性下降。

	\item \textbf{参数影响规律}：$\varepsilon = 0$（靠并列 Q 值随机选择提供自然探索）、$\alpha = 0.7$、$\gamma = 0.95$、$obs\_mul\_factor = 30$ 的组合在 Flappy Bird 任务中表现最优。$\varepsilon$ 过大和 $obs\_mul\_factor$ 过大是两个最主要的性能杀手。

	\item \textbf{奖励塑形}：bonus2 的奖励塑形能提供密集反馈并加速早期收敛，但在长周期训练中并未稳定超越基础死亡惩罚方案，说明塑形系数的设定需要与任务奖赏量级仔细匹配。

	\item \textbf{最终模型}：以 $\alpha=0.7, \gamma=0.95, \varepsilon=0.0, obs\_mul\_factor=30$、状态模式 \texttt{enhanced}、奖励模式 \texttt{death\_penalty} 训练的 \texttt{q\_best.pkl} 为最终提交模型。
\end{enumerate}
